...
- (void)setViewScrollingType:(UIView *)scrollableView{
    objc_setAssociatedObject(self, @selector(viewScrollingType), scrollableView, OBJC_ASSOCIATION_RETAIN);
}
- (NSLayoutConstraint *)viewScrollingType{
    
    return objc_getAssociatedObject(self, @selector(viewScrollingType));
}
...
- (void)handlePan:(UIPanGestureRecognizer*)gesture
{
    ...
    if([self.viewScrollingType isKindOfClass:[UITableView class]])
    {
...
- (void)updateNavbarAlpha:(CGFloat)delta
{
    ...
    if(alpha==0)
    {
        ...
        self.scrollableView.frame=CGRectMake(0,-self.navigationController.navigationBar.frame.size.height, superView.frame.size.width, superView.frame.size.height+self.navigationController.navigationBar.frame.size.height);
        
        
    }
    else{
      
        self.scrollableView.frame=CGRectMake(0,0, superView.frame.size.width, superView.frame.size.height);
